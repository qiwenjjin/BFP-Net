...
class Solver(BaseSolver):
    def __init__(self, cfg):
        ...
        # self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer,500,0.5,last_epoch=-1)
        #self.vggloss = make_loss('VGG54')
        if self.cfg['schedule']['use_CosAneal'] :
            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, self.cfg['schedule']['epoch_num'], self.cfg['schedule']['minimum'])
        else :
            self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer, self.cfg['schedule']['epoch_num'], self.cfg['schedule']['decay'], last_epoch=-1)
        self.log_name = self.cfg['dataset'] + '_' + str(self.cfg['data']['upsacle']) + '_' + str(self.timestamp)
        # save log
    ...
    def train(self): 
        with tqdm(total=len(self.train_loader), miniters=1,
            ...
            for iteration, batch in enumerate(self.train_loader, 1):
                ...
                #print(y.shape)
                #print(lms_image.shape, pan_image.shape)  ##(4,32,32)  (1,128,128)

                # print(y_amp.shape, gnd_amp.shape)
                # print(y_pha.shape, gnd_pha.shape)

                loss_f = self.loss(torch.log(y_amp + 1e-8), torch.log(gnd_amp + 1e-8)) + self.loss(y_pha, gnd_pha)

                #loss_f = self.loss(y_amp, gnd_amp) + self.loss(y_pha, gnd_pha)
