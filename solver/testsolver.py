...
class Testsolver(BaseSolver):
    ...
    def test(self):
        ...
        for batch in self.data_loader:
            ...
            if self.cfg['dataset'] == 'WV2_8band' :
                ...
                prediction = prediction.squeeze().clamp(0, 1).numpy().transpose(1,2,0)
                prediction = np.uint8(prediction*255).astype('uint8')
                for i in range(8):
                    if not os.path.exists('./saved_data/net_WV2_8/{}'.format(i+1)):
                        os.makedirs('./saved_data/net_WV2_8/{}'.format(i+1))
                        pass  
                    #if not os.path.exists('./saved_data/net_WV2_8/{}'.format(i+1)):
                        #os.makedirs('./saved_data/net_WV2_8/{}'.format(i+1))
                        #pass
                    imageio.imwrite('./saved_data/net_WV2_8/{}/{}.tif'.format(i+1, name[0][0:-4]), prediction[:, :, i])
            else :
    ...
    def eval(self):
        ...
        for batch in self.data_loader:
            ...
            lms_image, pan_image, bms_image, name = Variable(batch[0]), Variable(batch[1]), Variable(batch[2]), (batch[3])
            if self.cuda:
                lms_image = lms_image.cuda(self.gpu_ids[0])
                pan_image = pan_image.cuda(self.gpu_ids[0])
                bms_image = bms_image.cuda(self.gpu_ids[0])
